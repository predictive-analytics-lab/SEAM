...
class BatchNorm2dFixed(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        return F.batch_norm(
            input, self.running_mean, self.running_var, self.weight, self.bias, False, eps=self.eps
        )
...
class ExtractAffinityLabelInRadius:
    ...
    def __call__(self, label):
        ...
        fg_pos_affinity_label = np.logical_and(
            np.logical_and(pos_affinity_label, np.not_equal(bc_labels_from, 0)), concat_valid_pair
        ).astype(np.float32)
