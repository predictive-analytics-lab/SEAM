...
if __name__ == "__main__":
    ...
    infer_dataset = voc12.data.VOC12ImageDataset(
        ...
        transform=torchvision.transforms.Compose([np.asarray, model.normalize, imutils.HWC_to_CHW]),
    )
    ...
    for iter, (name, img) in enumerate(infer_data_loader):
        ...
        with torch.no_grad():
            ...
            if args.crf:
                img_8 = (
                    img[0].numpy().transpose((1, 2, 0))
                )  # F.interpolate(img, (dheight,dwidth), mode='bilinear')[0].numpy().transpose((1,2,0))
                img_8 = np.ascontiguousarray(img_8)
                mean = (0.485, 0.456, 0.406)
                std = (0.229, 0.224, 0.225)
                img_8[:, :, 0] = (img_8[:, :, 0] * std[0] + mean[0]) * 255
